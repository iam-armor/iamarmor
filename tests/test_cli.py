...
class TestLintBasic:
    def test_fail_tf_exits_1_and_shows_finding(self):
        result = runner.invoke(app, ["lint", str(IAM001_FAIL)])
    ...
    def test_bad_path_exits_2(self):
        result = runner.invoke(app, ["lint", "/nonexistent/path/does/not/exist"])
...
class TestLintJsonFormat:
    def test_json_output_valid_json(self):
        result = runner.invoke(app, ["lint", "--format", "json", str(IAM001_FAIL)])
        assert result.exit_code == 1
    ...
    def test_json_pass_exits_0_empty_findings(self):
        result = runner.invoke(app, ["lint", "--format", "json", str(IAM001_PASS)])
        assert result.exit_code == 0
        data = json.loads(result.stdout)
...
class TestLintSelect:
    def test_select_runs_only_specified_rule(self):
        result = runner.invoke(app, ["lint", "--select", "IAM001", str(IAM001_FAIL)])
        assert result.exit_code == 1
        assert "IAM001" in result.stdout

    def test_select_nonmatching_rule_exits_0(self):
        result = runner.invoke(app, ["lint", "--select", "IAM002", str(IAM001_FAIL)])
        assert result.exit_code == 0

    def test_ignore_iam001_exits_0_on_fail_tf(self):
        result = runner.invoke(app, ["lint", "--ignore", "IAM001", str(IAM001_FAIL)])
        assert result.exit_code == 0


class TestLintSeverityAndFailOn:
    def test_severity_threshold_filters_out_lower(self):
        ...
        result = runner.invoke(
            app,
            ["lint", "--select", "IAM003", "--severity-threshold", "high", "--fail-on", "high",
             str(iam003_fail)],
        )
    ...
    def test_fail_on_none_always_exits_0(self):
        result = runner.invoke(
            app, ["lint", "--fail-on", "none", str(IAM001_FAIL)]
        )
        assert result.exit_code == 0

    def test_fail_on_high_exits_1_for_high_finding(self):
        result = runner.invoke(
            app, ["lint", "--fail-on", "high", str(IAM001_FAIL)]
        )
        assert result.exit_code == 1
    ...
    def test_fail_on_critical_exits_0_for_high_finding(self):
        ...
        result = runner.invoke(
            app, ["lint", "--fail-on", "critical", str(IAM001_FAIL)]
        )
        assert result.exit_code == 0


class TestLintNoConfig:
    ...
    def test_directory_scan(self):
        result = runner.invoke(app, ["lint", str(IAM001_DIR)])
